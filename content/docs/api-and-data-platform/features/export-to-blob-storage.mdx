...
| `usage`         | `usage_details`, `cost_details`, `total_cost`, `usage_pricing_tier_name`                                     | Yes                        |
...
<Callout type="info">
  Per-unit pricing fields (`input_price`, `output_price`, `total_price`) live in the `model` group — they come from the matched model definition. Deselecting `model` skips the worker-side model pricing lookup entirely. The `usage_pricing_tier_name` field stays in the `usage` group.
</Callout>
